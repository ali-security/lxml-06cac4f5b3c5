...
# The last libxml2-win-binaries release that ships a complete "vs2008" set
# (iconv 1.14, libxml2 2.10.3, libxslt 1.1.37, zlib 1.2.12, win32 + win64).
# Those objects were built with the same VS 9.0 compiler that the
# "VCForPython27" MSI installs, which is what CPython 2.7 extensions need.
# The scan for the newest releases never reaches back this far, so pin it.
PY2_WIN_BINARIES_RELEASE_TAG = "2023.03.26"
...
def find_windows_binaries_release():
    """Find the libxml2-win-binaries release to use for this interpreter."""
    api_url = "https://api.github.com/repos/lxml/libxml2-win-binaries/releases"
    api_token = os.environ.get("GITHUB_API_TOKEN")

    if sys.version_info[0] < 3:
        # CPython 2.7 needs the VS 2008 built objects, which only exist up to
        # a specific old release => request that release directly.
        release, _ = read_url(
            "%s/tags/%s" % (api_url, PY2_WIN_BINARIES_RELEASE_TAG),
